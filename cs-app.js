  ...
  function finishCalc(recognized, manual, taxRate, voucher){
    ...
    function addRow(it, isManual){
      ...
      if(base > 0 || it.bogoMaster){
        rows.push({ tr: it.tr || null, name: it.name || '', base: base,
    ...
    manual.forEach(function(m){ addRow({tr:m.tr, name:m.name, price:m.price, qty:m.qty}, true); });
    ...
    order.forEach(function(r){
      ...
      lastVoucherBreakdown.push({name:r.name, base:r.base, covered:r.covered,
    ...
    if(unknown.length){
      ...
      document.getElementById('modalRows').innerHTML = unknown.map(function(u,i){
        return '<div class="mrow">'
             ...
             + '<input type="number" min="0" step="1" placeholder="$ price" data-i="'+i+'">'
             + '</div>';
      }).join('');
    } else {
  ...
  function confirmCalc(){
    ...
    document.querySelectorAll('#modalRows input').forEach(function(inp){
      ...
      manual.push({tr:pendingUnknown[i].tr, name:pendingUnknown[i].name, price:v, qty:pendingUnknown[i].qty});
    });
